...
bool ReadString(const std::vector<std::uint8_t>& frame, std::size_t& offset, std::string& out) {
    ...
    // Same sanity bound used elsewhere in this project for
    // decoder-side length fields (JournalRecordCodec, FileBackingStore):
    // refuse to trust an absurd length rather than attempting a huge
    // allocation driven by corrupt/hostile input.
    constexpr std::uint32_t kMaxReasonableLength = 16u * 1024u * 1024u;
    if (length > kMaxReasonableLength || offset + length > frame.size()) return false;
